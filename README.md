...
**Example:** [`Formulas/ripgrep.json`](Formulas/ripgrep.json) is a real, installable manifest for [ripgrep](https://github.com/BurntSushi/ripgrep) using the **Linux x86_64 musl** GitHub release asset. tars currently uses one `url` per formula (no per-OS matrix), so other platforms need a different asset URL and `sha256` (often a separate formula name or your own tap).
